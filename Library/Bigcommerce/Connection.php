...
class Connection
{
    ...
    public static function verifySignedRequest($signedRequest,$clientSecret){
        ...
        if (function_exists('hash_equals')) {
            if (!hash_equals($expectedSignature, $signature)) {
      		    error_log('Bad signed request from Bigcommerce1!');
                return false;
            }
        }else{
            if (!self::hash_equals($expectedSignature, $signature)) {
                error_log('Bad signed request from Bigcommerce2!');
                return false;
            }
        }
        return $data;
    }
    private static function hash_equals($known_string, $user_string){
